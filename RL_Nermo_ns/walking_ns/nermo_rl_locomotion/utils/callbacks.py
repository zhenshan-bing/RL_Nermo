...
# Callbacks from rl-baselines3-zoo (https://github.com/DLR-RM/rl-baselines3-zoo/blob/v1.3.0/utils/callbacks.py)
...
# Source: https://stable-baselines3.readthedocs.io/en/master/guide/tensorboard.html#logging-videos
class VideoRecorderCallback(BaseCallback):
    def __init__(self, eval_env: gym.Env, recording_freq: int, n_eval_episodes: int = 1, deterministic: bool = True):
        """
        Records a video of an agent's trajectory traversing ``eval_env`` and logs it to TensorBoard

        :param eval_env: A gym environment from which the trajectory is recorded
        :param recording_freq: Render the agent's trajectory every recording_freq call of the callback.
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self._recording_freq == 0:
            print("Recording video ...")
            # Fix based on https://github.com/openai/mujoco-py/issues/390#issuecomment-525385434
            # glfw_context = GlfwContext(offscreen=True)
            # FIXME: "fix" above makes it possible to record a video but somehow results in the programm not being able to continue
            screens = []
            ...
            print("Done recording video. Logging video to tensorboard...")
            ...
            print("Done logging video to tensorboard.")
